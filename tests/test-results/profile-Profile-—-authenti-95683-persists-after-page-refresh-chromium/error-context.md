...
Locator: getByText(/RefreshCheck/i)
Expected: visible
Error: strict mode violation: getByText(/RefreshCheck/i) resolved to 2 elements:
    1) <h1 class="font-[var(--font-display)] text-4xl font-bold text-[var(--color-text-primary)]">Hi, RefreshCheck</h1> aka getByRole('heading', { name: 'Hi, RefreshCheck' })
    2) <p class="mt-2 text-base font-semibold text-[var(--green-deeper)]">RefreshCheck</p> aka getByText('RefreshCheck', { exact: true })

Call log:
  - Expect "toBeVisible" with timeout 8000ms
  - waiting for getByText(/RefreshCheck/i)
...
> 88  |     await loginViaUI(page, email, TEST_PASSWORD);
  ...
  162 | 
  163 |     await page.request.post("/api/avatar", {
  164 |       data: { hat: "grad_cap", accessory: null, badge: "fire" },
  165 |     });
  166 | 
  167 |     const profileData = await getUserProfile(userId);
  168 |     const avatar = profileData.avatar as { hat?: string; badge?: string };
  169 |     expect(avatar?.hat).toBe("grad_cap");
  170 |   });
  171 | 
  172 |   test("unauthenticated POST /api/avatar returns 401", async ({ page }) => {
  173 |     const response = await page.request.post("/api/avatar", {
  174 |       data: { hat: "cap" },
  175 |     });
